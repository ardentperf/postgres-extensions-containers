...
provisionally populate or augment the owning packages' `licenseDeclared`
fields as a conservative `AND` expression, while preserving any existing
declared expression. This is a scanner-oriented license inventory projection,
not a definitive statement of the package authors' legally declared license;
the file-level fields remain the detailed evidence and `licenseConcluded` is
unchanged.
